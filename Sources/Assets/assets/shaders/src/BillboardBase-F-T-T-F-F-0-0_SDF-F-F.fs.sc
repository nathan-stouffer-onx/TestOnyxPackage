$input v_position, v_texcoord7, v_texcoord6, v_texcoord5, v_texcoord4
//includes
...
rgba[2] = color.x;
rgba[3] = color.w;
float distance = rgba[index] * 0.97;
float smoothing = 16.0 * length(fwidth(sdf_tex0.xyz)) / sqrt(12.0) * u_distanceMultiplier;
float outlineWidth = u_outlineWidth * smoothing * 2.0;
float outerEdgeCenter = 0.5 - outlineWidth;
